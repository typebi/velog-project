package com.typebi.spring.comment.controller
...
import com.typebi.spring.comment.service.CommentService
...
class CommentController(
    ...
    fun getCommentById(@PathVariable(name = "commentId") commentId: Long): ResponseEntity<ApiResponse<CommentResponseDTO>> {
        ...
        val response =
            ApiResponse(true, commentDTO, "Comment with ID $commentId retrieved successfully", HttpStatus.OK.value())
        return ResponseEntity.ok(response)
    }
    ...
    fun updateCommentById(
        ...
        val response =
            ApiResponse(true, commentDTO, "Comment with ID $commentId updated successfully", HttpStatus.OK.value())
        return ResponseEntity.ok(response)
    }
    ...
    fun deleteCommentById(@PathVariable(name = "commentId") commentId: Long): ResponseEntity<ApiResponse<Unit>> {
        ...
        val response =
            ApiResponse(result, Unit, "Comment with ID $commentId deleted successfully", HttpStatus.NO_CONTENT.value())
        return ResponseEntity.ok(response)
    }
}